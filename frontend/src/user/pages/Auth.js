  ...
  const authSubmitHandler = async (event) => {
    ...
    if (isLoginMode) {
      ...
      try {
        ...
        const result = await sendRequest(`${process.env.REACT_APP_BACKEND_URL}/users/signup`,
          'POST',
          formData
        )
        auth.login(result.userId, result.token)
  ...
  return (
    <>
      ...
      <Card className='authentication'>
        ...
        <Button inverse onClick={swithchModeHandler}>SWITCH TO {isLoginMode ? 'SIGN UP' : 'LOGIN'}</Button>
      </Card>
    </>
  )
}
